...
OUT_DIR   = r"C:\ML\Results\Training and Testing (My_SPLIT 80-20) - RF"  # saída
...
def build_preprocessor_from_train(X_train_raw: pd.DataFrame):
    ...
    # Se houver boolean, trata como categórico (OneHot) para manter semântica
...
def align_test_to_train_schema(X_train_raw: pd.DataFrame, X_test_raw: pd.DataFrame, out_dir: str):
    """
    FIX do crash SEM degradar qualidade:
      - garante que o teste tenha TODAS as colunas do treino (mesma ordem),
        criando as faltantes como NaN (para o imputer preencher com estatística do treino).
      - remove colunas extras do teste (o pipeline não conhece).
      - salva diagnósticos de schema/diffs e %missing (para evidenciar domain shift).
    """
    ...
    pd.DataFrame({"missing_in_test": missing_in_test}).to_csv(os.path.join(out_dir, "schema_missing_in_test.csv"), index=False)
    pd.DataFrame({"extra_in_test": extra_in_test}).to_csv(os.path.join(out_dir, "schema_extra_in_test.csv"), index=False)

    if missing_in_test:
        log(f"[WARN] Test is missing {len(missing_in_test)} train columns. Adding them as NaN (imputer will fill).")
    ...
    # reindex: adiciona faltantes como NaN e dropa extras
    X_test_aligned = X_test_raw.reindex(columns=train_cols, fill_value=np.nan)

    # %missing no teste após alinhamento (para ver se “sumiram” features relevantes)
    missing_pct = (X_test_aligned.isna().mean() * 100.0).sort_values(ascending=False)
    missing_pct.to_csv(os.path.join(out_dir, "test_missing_percent_after_align.csv"), header=["missing_percent"])
...
def choose_shap_sample(X_raw: pd.DataFrame, y_true: np.ndarray, max_rows: int = 5000, random_state: int = 42):
    """
    Amostra balanceada quando possível (melhora interpretabilidade e evita SHAP “mudo”).
    """
    n = len(X_raw)
    if n <= max_rows:
        return X_raw, y_true
    ...
    if n_classes == 0:
        return X_raw.sample(n=max_rows, random_state=random_state), y_true[:max_rows]

    per_class = max_rows // n_classes
    sampled = []
    for _, g in groups:
        take = min(len(g), per_class)
        sampled.append(g.sample(n=take, random_state=random_state))
    ...
    if len(samp) < max_rows:
        ...
        if len(rest) > 0 and remaining > 0:
            samp = pd.concat(
                [samp, rest.sample(n=min(remaining, len(rest)), random_state=random_state)],
                ignore_index=True
            )
...
def extract_feature_names(preprocessor, num_cols, cat_cols):
    ...
    try:
        ...
        cat_feature_names = [f"cat_{i}" for i in range(0)]
    return list(num_cols) + cat_feature_names


def run_model(train_files, test_files, output_path: str, trees: int = 3000):
    ...
    # ================
    # FIX DO CRASH (sem “gambiarra”):
    # alinha o schema do TESTE ao TREINO criando faltantes como NaN
    # ================
    X_test_raw = align_test_to_train_schema(X_train_raw, X_test_raw, output_path)

    # preprocess definido a partir do TREINO
    ...
    log(f"Training main model (n_estimators={trees})...")
    clf.fit(X_train_raw, y_train)
    log("Main model trained.")

    log("Predicting...")
    y_pred = clf.predict(X_test_raw).astype(int)
    log(f"Predictions: {len(y_pred)}")

    # salvar predições + diagnóstico (evita interpretar matriz “estranha”)
    ...
    log(f"SHAP: training small model (n_estimators={SHAP_TREES}) for explainability...")
    model_shap = RandomForestClassifier(
        n_estimators=SHAP_TREES,
        random_state=50,
        class_weight="balanced",
        n_jobs=-1
    )

    # novo pipeline SHAP (mesmo preprocessor fitado no treino)
    preprocessor_shap, num_cols_s, cat_cols_s = build_preprocessor_from_train(X_train_raw)
    clf_shap = Pipeline(steps=[
        ("preprocess", preprocessor_shap),
        ("model", model_shap)
    ])
    clf_shap.fit(X_train_raw, y_train)

    # transforma para o espaço final (após preprocess) e usa o estimator final
    X_shap = clf_shap.named_steps["preprocess"].transform(X_shap_raw)
    feature_names = extract_feature_names(clf_shap.named_steps["preprocess"], num_cols_s, cat_cols_s)

    log(f"SHAP: computing on {X_shap.shape[0]} rows and {X_shap.shape[1]} features...")
    explainer = shap.TreeExplainer(clf_shap.named_steps["model"])
    ...
    plt.figure()
    shap.summary_plot(shap_values, features=X_shap, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(shap_bar_png, bbox_inches="tight")
    plt.close()

    shap_bee_png = os.path.join(output_path, "shap_beeswarm_positive.png")
    # se multiclass, usa classe 1 quando existir; senão usa 0
    ...
    plt.figure()
    shap.summary_plot(sv_pos, features=X_shap, feature_names=feature_names, show=False)
...
def main():
    ...
    run_model(train_files, test_files, OUT_DIR, TREES)
