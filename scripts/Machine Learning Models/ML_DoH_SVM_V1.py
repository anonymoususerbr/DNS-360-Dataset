...
OUT_DIR = r"C:\Users\cborges\OneDrive - Fortinet\Documents\Studies\Doctorade UFU\Research\Development\Dataset\My Dataset\CSVs\Results\SVM_DoH - Train (My) and Test (My)"
...
def save_svm_coefficients(model, feature_names, le: LabelEncoder, out_dir: str):
    ...
    if coef.shape[0] == 1:
        class_name = le.classes_[1] if len(le.classes_) == 2 else "positive_class"
        ...
        for class_idx, class_name in enumerate(le.classes_):
            for feature, value in zip(feature_names, coef[class_idx]):
                rows.append({
                    "class": class_name,
                    "feature": feature,
                    "coefficient": float(value),
    ...
    df_coef = pd.DataFrame(rows).sort_values(
        ["class", "abs_coefficient"],
        ascending=[True, False]
    )

    df_coef.to_csv(os.path.join(out_dir, "svm_coefficients.csv"), index=False)

    df_top = df_coef.sort_values("abs_coefficient", ascending=False).head(200)
    df_top.to_csv(os.path.join(out_dir, "svm_coefficients_top200.csv"), index=False)
...
def main():
    ...
    log(f"Training Linear SVM (C={SVM_C}, max_iter={SVM_MAX_ITER})...")
